 ...
 */
@WebServlet("/DBCheckOut")
public class DBCheckOut extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("taiKhoan1")!=null) {
			...
			long idCurrentHoaDon = Long.parseLong(request.getParameter("id"));
			try {
			...
			request.getRequestDispatcher("DBCheckout.jsp").forward(request, response);
		}else {
			response.sendRedirect("index");
		}
	}
